...
This report was prepared to the attention of the United Nations. It analyzes the relationship between a country's GDP, its life expectancy, and CO2 emissions. Our goal is to determine to what degree a country's economic strenght or weakness may be related to its public health status and impact on climate pollution. We hypothesize that both life expectancy and C02 emissions increase with a country's GDP.
...
minGDP <- gapminder_1997 %>%  summarize(round(min(gdpPercap))) %>% 
  pull()  # pull() is to tell R not to create a "tibble" but a "value"
maxGDP <- gapminder_1997 %>%  summarize(round(max(gdpPercap))) %>% 
  pull()  
```

The above plot shows the relationship between GDP per capita nad life expectancy for a total of `r nCountries` countries. For this set of countries, economic wealth ranged from a minimum of USD `r minGDP` to a maximum of USD `r maxGDP`.
